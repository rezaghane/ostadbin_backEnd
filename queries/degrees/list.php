...
//***************************** SQL ALL *******************************************
$sqlALL = "SELECT * FROM degrees WHERE ".$wher;
$rows = $db->rowsCount($sqlALL);
$rows_int = (int)($rows / $_GET['n']);
$totalPages = ($rows_int == $rows / $_GET['n']) ? $rows_int : $rows_int+1;
//***************************** SQL *******************************************
$sql = "SELECT `id`, `name`, `status` FROM degrees WHERE ".$wher." ORDER BY ".$sort.$lmt;
$db->query($sql);
$data = $db->resultSet();
if(sizeof($data)>0){
  $DATA = array();
  for($i=0; $i<sizeof($data); $i++){
    $sqlT = "SELECT COUNT(*) AS cnt FROM teachers WHERE CONCAT(',' , degrees , ',') LIKE '%,".$data[$i]->id.",%' ";
    $db->query($sqlT);
    $teacherC = $db->resultSet();
    // _________________________________________________________________________
    $sqlT = "SELECT id, name FROM category WHERE status=1 AND CONCAT(',' , degrees , ',') LIKE '%,".$data[$i]->id.",%' ";
    $db->query($sqlT);
    $category = $db->resultSet();
    // _________________________________________________________________________
    $sqlT = "SELECT id, name FROM category WHERE status=1";
    $db->query($sqlT);
    $categoryAll = $db->resultSet();
    // _________________________________________________________________________
    $D = [
      "id"=> $data[$i]->id,
