...
if($_POST['role']==3){
	...
	$wher .= " AND (dsc  LIKE '%".$_GET['search']."%' OR title  LIKE '%".$_GET['search']."%' OR href  LIKE '%".$_GET['search']."%') ";
	//***************************** LIMIT *******************************************
	$lmt = ( $_POST['role'] ==3 ) ? " LIMIT ".(($_GET['p']-1) * $_GET['n'])." , ".($_GET['n']) : ' ';
	//***************************** SQL ALL *******************************************
	$sqlALL = "SELECT * FROM settings WHERE ".$wher;
	$rows = $db->rowsCount($sqlALL);
	$rows_int = (int)($rows / $_GET['n']);
	$totalPages = ($rows_int == $rows / $_GET['n']) ? $rows_int : $rows_int+1;
	//***************************** SQL *******************************************
	$sql = "SELECT * FROM settings WHERE ".$wher." ORDER BY ".$sort.$lmt;
	$db->query($sql);
	$data = $db->resultSet();
	if(sizeof($data)>0){
	  $DATA = array();
	...
	$sql = "SELECT `id`, `title`, `dsc`, `href` FROM `settings` WHERE `status`=1";
	$db->query($sql);
	$arr = $db->resultSet();
	$cod = 200;
	$txt = 'OK';
